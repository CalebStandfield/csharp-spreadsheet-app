...
public class DependencyGraph
{
    ...
    {
        if (Dependents.TryGetValue(nodeName, out HashSet<string>? dependents))
            return dependents;
        return [];
    }
    ...
    {
        if (Dependees.TryGetValue(nodeName, out HashSet<string>? dependees))
            return dependees;
        return [];
    }
    ...
    public void AddDependency(string dependee, string dependent)
    {
        ...
        if (Dependents[dependee].Add(dependent))
        {
            size++;
    ...
    {
        if (HasDependents(dependee) && Dependents[dependee].Remove(dependent))
        {
            size--;
            //Removes empty HashSets from Dependees and Dependents
    ...
    {
        if (Dependents.TryGetValue(nodeName, out HashSet<string>? oldDependents))
            foreach (string strOld in oldDependents)
                RemoveDependency(nodeName, strOld);
        foreach(string strNew in newDependents)
            AddDependency(nodeName, strNew);
    }
    ...
    {
        if (Dependees.TryGetValue(nodeName, out HashSet<string>? oldDependees))
            foreach (string strOld in oldDependees)
                RemoveDependency(strOld, nodeName);
        foreach (string strNew in newDependees)
            AddDependency(strNew, nodeName);
    }
}